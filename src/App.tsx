import React, { useCallback, useRef } from 'react';
import { useTodo, useAddTodo, useRemoveTodo, TodosProvider } from './useTodos';
import './App.css';

const Heading = ({title}: { title: string }) => (
  <h2>{title}</h2>
)

const Box: React.FunctionComponent<{ children:string }> = ({children}) => (
  <div
    style={{ 
      padding: '1rem',
      fontWeight: 'bold'
     }}
  >
    {children}
  </div>
) 


const Button: React.FunctionComponent<React.DetailedHTMLProps<React.ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement> & {
  title?: string
}> 
= ({ title, children, style, ...rest }) => (
  <button {...rest} style={{ 
    ...style,
    backgroundColor: 'steelblue',
    color: 'whitesmoke'
   }}>{title ?? children}</button>
)

function UL<T>({
  items,
  render,
  itemClick
}: React.DetailedHTMLProps<React.HTMLAttributes<HTMLUListElement>, HTMLUListElement>
& {
  items: T[],
  render: (item: T) => React.ReactNode,
  itemClick: (item: T) => void
}) {
  return (
    <ul>
      {items.map((item, index) => (
        <li onClick={() => itemClick(item)} key={[index, item].join(':')}>{render(item)}</li>
      ))}
    </ul>
  )
}

function App() {
  const todos = useTodo()
  const addTodo = useAddTodo()
  const removeTodo = useRemoveTodo()
  const newTodoRef = useRef<HTMLInputElement>(null)
  const onAddTodo = useCallback(() => {
    if(newTodoRef.current){
      addTodo(newTodoRef.current.value)
      newTodoRef.current.value = ''
    }
  }, [addTodo])

  return (
    <div>
      <Heading title='Introducing'/>
      <Box children='Hello There'/>
      <Heading title='Todos'/>
      <UL 
        items={todos}
        itemClick={() => {}}
        render={(todo) => (
          <>
            {todo.text}
            <button onClick={() => removeTodo(todo.id)}>Remove</button>
          </>
        )}
      />
      <div>
        <input type="text" ref={newTodoRef} />
        <Button onClick={onAddTodo}>Add Todo</Button>
      </div>
    </div>
  );
}

const ShowTodos = () => {
  const todos = useTodo()
  return (
    <UL 
        items={todos}
        itemClick={() => {}}
        render={(todo) => (
          <>
            {todo.text}
          </>
        )}
      />
  )
}

const AppWrapper = () => (
  <TodosProvider initialTodos={[{ id: 0, text: 'Hello There useContextTodo', done: false}]}>
    <div style={{ 
      display: 'grid',
      gridTemplateColumns: '50% 50%'
    }}>
      <App></App>
      <ShowTodos></ShowTodos>
    </div>
  </TodosProvider>
)

export default AppWrapper;
